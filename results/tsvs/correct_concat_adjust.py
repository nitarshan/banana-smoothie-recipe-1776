...
dfs = []
for dataset_size in [6_250, 12_500, 25_000, 50_000]:
  dfs.append(pd.read_csv(f'./results/tsvs/nin_svhn.{dataset_size}.tsv', sep='\t', index_col=0))
pd.concat(dfs).to_csv(f'./results/tsvs/nin_svhn.tsv', sep='\t', float_format="%g")

# Adjust measures accounting for dataset size
df = pd.read_csv('./results/tsvs/nin_svhn.tsv', sep='\t', index_col=0)
...
df.to_csv(f'./results/tsvs/nin_svhn_adjusted.tsv', sep='\t', float_format="%g")
