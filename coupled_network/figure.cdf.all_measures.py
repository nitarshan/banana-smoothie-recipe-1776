...
def get_all_losses(data, hp, env_weights, env_losses, hps, hp_combo_id, min_weight=20.):
    """
    Gather the loss in each environment where some hyperparameter is varied from v1 to v2 and the remaining HPs are kept fixed.
    In each environment, the loss is given by the expected loss of importance sampling estimator over all random seeds.
    ...
    for i, v1 in enumerate(values):
        ...
        for j, v2 in enumerate(values):
            ...
            v1_combos_, v2_combos_, v1_idx, v2_idx = zip(*[(v1c, v2c, hp_combo_id[v1c], hp_combo_id[v2c]) for v1c, v2c in zip(v1_combos, v2_combos) if v2c in hp_combo_id])
...
def make_figure(datasets, tau=0, min_weight=10):
    ...
    extra_info = pickle.load(open(ENVIRONMENT_CACHE_PATH + "/env_losses__tau%d__%s.extra_info.pkl" % (tau, data_key), "rb"))
    ...
    for c in env_losses:
        ...
        for hp in extra_info["hps"]:
            complexity_losses_per_hp[c][hp] = get_all_losses(data, hp, env_weights=env_weights, env_losses=env_losses[c], 
                                                            hp_combo_id=extra_info["hp_combo_id"], hps=extra_info["hps"], 
                                                            min_weight=min_weight)
        complexity_losses_per_hp[c]["all"] = np.hstack([complexity_losses_per_hp[c][h] for h in complexity_losses_per_hp[c]])

    ordered_measures = np.array(list(env_losses.keys()))[np.argsort([np.mean(complexity_losses_per_hp[c]["all"]) for c in env_losses])].tolist()  # Order by mean
    ...
    for ax, hp in zip(axes, ordered_hps):
        ...
        ax.set_ylabel(pretty_hps[hp] + "\n(%d)" % (len(complexity_losses_per_hp[list(complexity_losses_per_hp.keys())[0]][hp])),
                    fontsize=8)
        ax.set_ylim(0, 101)
    ...
    plt.savefig("figure.%s_tau%d_mw_%f_cdf_per_hp.pdf" % (data_key, tau, min_weight), bbox_inches="tight")
