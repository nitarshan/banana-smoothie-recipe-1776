...
class BaseLogger(object):
    ...
    def log_batch_end(self, cfg: EConfig, state: ETrainingState, cross_entropy: torch.Tensor, complexity: torch.Tensor,
                      loss: torch.Tensor, constraint: torch.Tensor):
...
class Printer(object):
    ...
    def epoch_metrics(self, epoch: int, train_eval, val_eval) -> None:
        if self.verbosity >= Verbosity.EPOCH:
            print(
                '[{}][Epoch {}][GEN L: {:.2g} E: {:.2f}pp][{} L: {:.4g}, C: {:.4g}, A: {:.0f}/{} ({:.2f}%)][{} L: {:.4g}, C: {:.4g}, A: {:.0f}/{} ({:.2f}%)]'.format(
                    ...
                    DatasetSubsetType.TRAIN.name, train_eval[1], train_eval[2], train_eval[3], train_eval[4],
                    100. * train_eval[0]))
